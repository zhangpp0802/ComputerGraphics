...
// Constructor for a new particle system.
  ...
  /** ********************************* Here to add more uniforms **/
    /** ***********************************/
    this.u_runMode;				        // GPU location for 'u_runMode' uniform
    // this.u_ballShift;				    // GPU location for 'u_ballShift' uniform
...
PartSys.prototype.initBouncy3D = function(count) { 
  ...
  this.s1 =    new Float32Array(this.partCount * PART_MAXVAR); // s1 for current timestep
  this.s2 =    new Float32Array(this.partCount * PART_MAXVAR); // s2 for next timestep
  this.s1dot = new Float32Array(this.partCount * PART_MAXVAR);  // s1dot for its time-derivative
...
// That's OK for most particle parameters, but these need non-zero defaults:
  ...
  for(var i = 0; i < this.partCount; i += 1, j+= PART_MAXVAR) {
    ...
    this.s1[j + PART_LIFELEFT] = 20;// 10 to 20
    this.s1[j + PART_RENDMODE] = 0.0;
    this.s1[j + PART_AGE] = 30;
    //----------------------------
...
PartSys.prototype.initSpringPair = function() { 
  ...
  this.INIT_VEL =  0.0;		// initial velocity in meters/sec.
	                  // adjust by ++Start, --Start buttons. Original value 
										// was 0.15 meters per timestep; multiply by 60 to get
                    // meters per second.
  this.drag = 0.985;// units-free air-drag (scales velocity); adjust by d/D keys
...
//    console.log("fList[k].forceType:", fList[k].forceType);
    ...
    switch(fList[k].forceType) {    // what kind of force should we apply?
      ...
      case F_DRAG:      // viscous drag: force = -K_drag * velocity.
        ...
        break;
      case F_SPRING:
        var p1 = new Vector3([s[PART_XPOS],s[PART_YPOS],s[PART_ZPOS]]);
        var p2 = new Vector3([s[PART_MAXVAR + PART_XPOS],s[PART_MAXVAR + PART_YPOS],s[PART_MAXVAR + PART_ZPOS]]);
        res = pointsDist(p2,p1);
        dir = [res[2].elements[0],res[2].elements[1],res[2].elements[2]];

        var stretch = res[1] - fList[k].K_restLength;
        var mag = stretch * fList[k].K_spring;

        for(var j = 0; j < 3; j ++){
          s[PART_X_FTOT + j] += mag * dir[j];
          s[PART_MAXVAR + PART_X_FTOT + j] -= mag * dir[j];
        }

        // Do Damping
        var netXVel = s[PART_MAXVAR + PART_XVEL] - s[PART_XVEL];
        var netYVel = s[PART_MAXVAR + PART_YVEL] - s[PART_YVEL];
        var netZVel = s[PART_MAXVAR + PART_ZVEL] - s[PART_ZVEL];

        magD = netXVel * dir[0] + netYVel * dir[1] + netZVel * dir[2];
        magD *= fList[k].K_springDamp;

        for(var j = 0; j < 3; j ++){
          s[PART_X_FTOT + j] += magD * dir[j];
          s[PART_MAXVAR + PART_X_FTOT + j] -= magD * dir[j];
        }
        break;
...
function pointsDist(p1, p2){
  var sub = new Vector3([p1.elements[0] - p2.elements[0],p1.elements[1] - p2.elements[1],p1.elements[2] - p2.elements[2]])
  var dist = Math.sqrt(Math.pow(sub.elements[0],2) + Math.pow(sub.elements[1],2) + Math.pow(sub.elements[2],2));
  var dir = new Vector3([sub.elements[0]/(dist+NU_EPSILON),sub.elements[1]/(dist+NU_EPSILON),sub.elements[2]/(dist+NU_EPSILON)])
  return [sub, dist, dir]
}

PartSys.prototype.press = function(scale){
  this.s1[PART_XPOS] = this.s1[PART_XPOS] + 0.1 * scale;
  this.s1[PART_MAXVAR + PART_XPOS] = this.s1[PART_MAXVAR + PART_XPOS] - 0.1 * scale;
}
PartSys.prototype.stretch = function(scale){
  this.s1[PART_XPOS] = this.s1[PART_XPOS] - 0.1 * scale;
  this.s1[PART_MAXVAR + PART_XPOS] = this.s1[PART_MAXVAR + PART_XPOS] + 0.1 * scale;
}